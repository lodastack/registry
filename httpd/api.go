...
func (s *Service) handlerNsNew(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if err != nil {
		ReturnServerError(w, fmt.Errorf("Create group of ns %s fail: %s", gOpName, err.Error()))
		return
	}
	...
	if err != nil {
		ReturnServerError(w, fmt.Errorf("Create group of ns %s fail: %s", gDevName, err.Error()))
		return
	}
	ReturnOK(w, "success")
}
